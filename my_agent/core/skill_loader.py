...
CATEGORY_KEYWORDS: dict[str, list[str]] = {
    ...
    "社会观察": ["社会", "底层", "阶层", "纪实", "人性", "群体", "现实"],
    "战略决策": ["战略", "主要矛盾", "组织", "竞争", "实践", "持久战", "统一战线", "破局"],
    "创业产品": ["创业", "产品", "MVP", "用户", "市场", "商业", "增长"],
    "投资市场": ["A股", "投资", "股票", "市场", "蓝筹", "散户", "政策市", "估值", "股息"],
    "自我成长": ["成长", "自律", "读书", "健身", "低谷", "选择", "职业", "人生"],
}
...
def load_markdown_skill(path: Path) -> PersonaSkill:
    ...
    triggers = extract_list_after_labels(raw, ["触发词", "核心触发词"])
    suitable_for = extract_list_after_labels(raw, ["适合场景", "适合"])
    not_suitable_for = extract_list_after_labels(raw, ["不适合场景", "明确不激活", "局限"])
    categories = infer_categories(raw, suitable_for)
